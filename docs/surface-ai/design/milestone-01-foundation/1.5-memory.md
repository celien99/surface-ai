...
private:
    ...
    std::atomic<TaggedHead> free_list_head_{};  // 无锁栈头指针（tag 消除 ABA），见 9. Thread Model
};
...
classDiagram
    ...
    class GpuPool {
        ...
        -free_list_head_ atomic~TaggedHead~
...
**内部空闲链表用无锁栈（`std::atomic<TaggedHead>` + CAS）实现，拒绝 `std::mutex`。** 12MP 图像 × 16 路相机在高帧率下的 `Acquire`/`Release` 调用频率远高于 1.1 批次 `TypeRegistry::Resolve` 这类低频查询路径，锁竞争在这一频率下会成为热路径瓶颈——每次 `Acquire`/`Release` 都要经历一次可能阻塞的互斥锁获取，多个 Capture 线程同时申请时会互相排队等待，即使持锁时间很短，操作系统调度带来的上下文切换开销在高频场景下会被放大。无锁栈把"弹出/压入一个节点"实现为一个 CAS 重试循环，多个线程可以真正并发尝试，失败只是重试而不是排队等待被唤醒。

**头指针必须带一个单调递增的 tag（generation counter），拒绝裸 `std::atomic<Node*>` 头指针方案。** 裸指针头指针版本的 `PopFreeList`/`PushFreeList`（把"弹出"实现为读一次 `head`、CAS 成 `head->next`）存在经典的 ABA 问题：线程 A 读到 `old_head = node0`、算出 `node0->next = node1` 之后被调度器换出；在 A 恢复之前，其他线程可能先后把 `node0`、`node1` 都弹出并派发给两个正在使用中的 `PooledPtr`，随后 `node0` 被归还（`Push` 把 `head` 重新设回 `node0`，此时 `node0->next` 已经被改写为归还时读到的最新 `head`，但 A 栈上缓存的 `node1` 早已过期）；A 恢复后执行 `compare_exchange_weak(head, node0, node1)`，由于当前 `head` 恰好又是 `node0`，CAS 会成功，把 `head` 错误地设成 A 缓存的过期值 `node1`——而 `node1` 此刻仍被另一个 `PooledPtr` 正常持有，从未归还。后续的 `Acquire` 会把这个仍在使用中的 slab 再次派发出去，两个互不知情的调用者同时持有同一块内存，是一次真正的数据损坏而非仅理论风险，在高并发/长时间运行下必然触发。裸指针 CAS 只能保证"当前值等于期望值"，无法区分"值恰好又变回相同地址"与"这段时间里节点从未被移动过"这两种情况，这正是 ABA 问题的本质。

修复方案是把头指针从单一 `Node*` 换成一个不可再分的 `{Node* pointer; std::uint64_t tag;}` 组合（`TaggedHead`），每次成功的 `Push`/`Pop` 都把 `tag` 递增一次；`compare_exchange_weak` 现在同时比较指针与 `tag`，只要这段时间内发生过任意一次成功的入栈/出栈（不论操作的是不是同一个节点），`tag` 就一定不同，CAS 就会失败并重试，从根本上排除"指针值恰好绕回同一个地址却误判为未变化"的情况：

```cpp
// 头指针与单调递增 tag 打包成一个可原子读写的整体，128 位（64 位指针 + 64 位 tag），
// 需要平台支持双字 CAS（x86-64/ARM64 均支持，见 12. Future Extension 的可移植性备注）。
struct TaggedHead {
...
Node* PopFreeList(std::atomic<TaggedHead>& head) noexcept {
    TaggedHead old_head = head.load(std::memory_order_acquire);
    while (old_head.pointer != nullptr &&
           !head.compare_exchange_weak(
               old_head,
               TaggedHead{old_head.pointer->next, old_head.tag + 1},
               std::memory_order_acq_rel,
               std::memory_order_acquire)) {
        // old_head 被 CAS 自动刷新为当前观测到的最新 {pointer, tag}，继续下一轮尝试
    }
    return old_head.pointer;  // nullptr 表示空闲链表已空，Acquire 据此返回 Memory_PoolExhausted
}

// 入栈（Release/PooledPtr 析构内部）：同样是单个 CAS 重试循环
void PushFreeList(std::atomic<TaggedHead>& head, Node* node) noexcept {
    TaggedHead old_head = head.load(std::memory_order_acquire);
    TaggedHead new_head;
    do {
...
这两个函数各自只有一个 `while`/`do-while` 循环，循环体内不包含除 CAS 重试本身之外的条件判断分支，符合"CAS 重试循环写成单层循环，不与其他条件嵌套"的约束；空闲链表为空这一情况通过循环条件（`old_head.pointer != nullptr`）本身表达，不需要额外的 `if` 分层。`tag` 只用于让 CAS 的比较维度多一维、探测"期间是否发生过操作"，不参与任何业务判断，`Pop`/`Push` 之外的代码不读取也不依赖它的具体数值。

**上述 ABA 修复的措辞取代本文档先前版本中裸指针（`std::atomic<Node*>`/`std::atomic<void*>`）头指针的描述；本文档其余章节中出现的 `free_list_head_`/`atomic<void*>` 字段声明统一理解为本节定稿的 `std::atomic<TaggedHead>`，字段名不变，仅底层类型从裸指针变为 tagged 结构。** 这一订正发生在里程碑 1 代码阶段（Task 1 的 review 中发现），先前的裸指针版本从未被判定为"已实现且已验证正确"，只是文档阶段编写时未察觉 ABA 风险；订正不改变 `GpuPool`/`PinnedPool`/`IMemoryPool` 在 4. Interfaces 中对外暴露的任何签名（`free_list_head_` 是私有实现细节，从未出现在公开接口里），因此不构成对已冻结公开契约的破坏性变更。
...
- `TaggedHead`（`{Node* pointer; std::uint64_t tag;}`，16 字节）的无锁 CAS 依赖平台支持双字（128 位）原子比较交换（x86-64 的 `CMPXCHG16B`、ARM64 的 `CASP`/LL/SC 对）。需要注意的是，在 x86-64 上 `std::atomic<TaggedHead>::is_always_lock_free` 并非默认为 `true`：GCC/Clang 仅在开启 `-mcx16`（即允许发射 `CMPXCHG16B` 指令）时才认为 16 字节原子是无锁的，否则 `__atomic_always_lock_free(16, 0)` 为 `false`、static_assert 会构建失败。因此构建系统在 x86-64 目标上显式为编译含该头文件的 target 追加 `-mcx16`（见 `src/memory/CMakeLists.txt`、`tests/memory/CMakeLists.txt` 的按架构门控），ARM64 原生具备双字 LSE 原子、无需该标志。两个目标架构——Ubuntu x64（配 `-mcx16`）、未来 ARM64 兼容目标——上 `is_always_lock_free` 均为 `true`，本批次不做运行期回退到基于锁的实现；若未来出现本批次未预见的目标平台缺乏双字 CAS 支持，需要重新评估是否退回到"裸指针 + 独立 `epoch`/hazard pointer 方案"或直接接受该平台上退化为锁实现，不在本版本预先设计，待真实出现该平台需求时再评估。
