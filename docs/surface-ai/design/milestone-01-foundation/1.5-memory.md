...
- `TaggedHead`（`{Node* pointer; std::uint64_t tag;}`，16 字节）的无锁 CAS 依赖平台支持双字（128 位）原子比较交换（x86-64 的 `CMPXCHG16B`、ARM64 的 `CASP`/LL/SC 对，`std::atomic<TaggedHead>::is_lock_free()` 在两个目标架构——Ubuntu x64、未来 ARM64 兼容目标——上预期均为 `true`，本批次不做运行期回退到基于锁的实现）；若未来出现本批次未预见的目标平台缺乏双字 CAS 支持，需要重新评估是否退回到"裸指针 + 独立 `epoch`/hazard pointer 方案"或直接接受该平台上退化为锁实现，不在本版本预先设计，待真实出现该平台需求时再评估。
