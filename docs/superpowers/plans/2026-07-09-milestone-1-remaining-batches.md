...
- `ErrorCode` (`include/sai/core/error.h`) is a single flat enum; each task that introduces new error codes appends them at the end of the existing enum (never reorders, never touches other batches' members), matching the append-only rule in 1.6 batch's `## 6. Data Structure`. As landed, the enum contains, in append order: the existing `Core_*`/`Lifecycle_*`, then `Memory_*` (ArenaExhausted, RequestExceedsSlabSize, PoolExhausted), then `Plugin_*` (VersionIncompatible, CapabilityUnsupported, LicenseInvalid, CircularDependency), then `Runtime_*` (QueueFull, Cancelled, NodeNotFound), then `Infra_*` (LogSinkInitFailed, ConfigFileNotFound, ConfigParseError, ConfigValidationFailed, ConfigKeyNotFound, ConfigKeyTypeMismatch).
